...
from .models import BorrowTransaction, Equipment, Department


class BorrowForm(forms.ModelForm):
    class Meta:
        model  = BorrowTransaction
        fields = ['equipment', 'borrower_name', 'department', 'purpose',
                  'released_by', 'date_borrowed', 'expected_return']
        widgets = {
            'date_borrowed':   forms.DateTimeInput(attrs={'type': 'datetime-local'}, format='%Y-%m-%dT%H:%M'),
            'expected_return': forms.DateTimeInput(attrs={'type': 'datetime-local'}, format='%Y-%m-%dT%H:%M'),
...
class TransactionFilterForm(forms.Form):
    ...
    department = forms.ModelChoiceField(queryset=Department.objects.all(), required=False,
                     empty_label='All departments', widget=forms.Select(attrs={'class': 'form-select'}))
